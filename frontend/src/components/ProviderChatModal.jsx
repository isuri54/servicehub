...
const ProviderChatModal = ({ isOpen, onClose }) => {
  ...
  const fetchConversations = async () => {
    try {
      ...
      const res = await axios.get("/api/chat/provider/conversations", {
  ...
  const openChat = async (clientId, clientName, clientImage) => {
    setSelectedClient({ clientId, clientName, clientImage });
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`/api/chat/provider/${clientId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const sendReply = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post("/api/chat/provider/reply", {
      ...
      setNewMessage("");
      openChat(selectedClient.clientId, selectedClient.clientName, selectedClient.clientImage);
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl h-5/6 flex flex-col">
        <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-t-3xl p-6 text-white flex justify-between items-center">
          <h2 className="text-3xl font-bold">Client Messages</h2>
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="w-80 border-r border-gray-200 bg-gray-50 overflow-y-auto">
            <div className="p-4">
              <h3 className="font-bold text-lg text-gray-700">Active Chats</h3>
            </div>
            {loading ? (
              ...
              conversations.map((conv) => (
                <div
                  key={conv.clientId}
                  onClick={() => openChat(conv.clientId, conv.clientName, conv.clientImage)}
                  className={`p-4 flex items-center gap-4 cursor-pointer hover:bg-white transition ${selectedClient?.clientId === conv.clientId ? "bg-white border-l-4 border-green-600" : ""}`}
                >
                  <img
                    src={conv.clientImage || "/user.png"}
                    alt={conv.clientName}
            ...
            {selectedClient ? (
              <>
                <div className="bg-gray-100 p-4 border-b border-gray-200 flex items-center gap-4">
                  <img src={selectedClient.clientImage || "/user.png"} alt="" className="w-10 h-10 rounded-full" />
                  <h3 className="font-bold text-gray-800">{selectedClient.clientName}</h3>
                </div>
